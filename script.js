// Wartet bis PROXY aus browser.html geladen ist
let tabs = [];
let activeTabId = null;
let tabCounter = 0;

let tabsContainer, framesContainer, urlInput, feuerBtn, tabCountEl, startPage;

function initApp() {
    tabsContainer = document.getElementById('tabs');
    framesContainer = document.getElementById('framesContainer');
    urlInput = document.getElementById('urlInput');
    feuerBtn = document.getElementById('feuerBtn');
    ...
    function switchTab(tabId) {
        ...
        if (frame) { frame.classList.add('active'); urlInput.value = frame.src.replace(window.PROXY, '').replace('about:blank', ''); }
        activeTabId = tabId;
        if (startPage) startPage.classList.add('hidden');
    }
    ...
    function feuern() {
        ...
        if (needsProxy) {
            addTab(window.PROXY + url, lower.length > 20 ? lower.substring(0, 20) + '...' : lower);
        } else {
