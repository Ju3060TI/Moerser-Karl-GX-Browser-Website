...
// ============================================
// FACH-TITEL AUS URL AUSLESEN
// ============================================
const urlParams = new URLSearchParams(window.location.search);
const fach = urlParams.get('fach');
...
const fachTitelEl = document.getElementById('fachTitel');
if (fachTitelEl && fach && fachNamen[fach]) {
    fachTitelEl.textContent = fachNamen[fach];
} else if (fachTitelEl) {
    fachTitelEl.textContent = '';
}
...
function updateTabCount() {
    if (tabCountEl) tabCountEl.textContent = tabs.length + ' Tab' + (tabs.length !== 1 ? 's' : '');
}
