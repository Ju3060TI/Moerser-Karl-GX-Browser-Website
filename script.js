...
// === POPUPS ==const PROXY = '/.netlify/functions/proxy?url=';
let tabs = [];
let activeTabId = null;
let tabCounter = 0;
...
function feuern() {
    ...
    // GESPERRTE Seiten → Neuer Tab
    const blockedSites = [
        'youtube.com', 'youtu.be', 'netflix.com', 'twitch.tv', 'discord.com',
        'tiktok.com', 'instagram.com', 'snapchat.com', 'reddit.com',
        'chat.openai.com', 'claude.ai', 'grok.x.ai', 'gemini.google.com'
    ];
    const isBlocked = blockedSites.some(d => url.includes(d));
    ...
    // ALLES ANDERE → Durch Proxy (inkl. Cookie Clicker, Desert Order, 1v1 etc.)
